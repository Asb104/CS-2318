################################################################################
# Name:    <Andrew Baker>
# Class:   CS2318-003 (Assembly Language, Fall 2018)
...
####################(8)####################
					addi $a0, $sp, 97
					jal GetOneIntByVal
					lw $t3, 52($sp)	#valsToDo
					sub $t0, $t3, $t1	
					sll $t0, $t0, 2	#valsToDo - i
					addi $t3, $sp, 0	#intArr[0]
					add $t3, $t3, $t0	
					sw $v0, 0($t3)	
...
# Register Usage:
#################
# $t3: holds charPtr, avoiding clobbering
#################
#   cout << prompt;
#   cin >> *charVarToPutInPtr;

####################(7)####################
					move $t3, $a0	#t3 = $a0(charVar)
					move $a0, $a1	#a0 = prompt
					li $v0, 4
					syscall	
					li $v0, 12
					syscall	
					sb $v0, 0($t3)	#store char
#}
					jr $ra
